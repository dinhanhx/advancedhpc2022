...
def kuwahara_filter(rgb_src, rgb_dst, v_arr, window_size):
    ...
    for wi in range(i + 1 - window_size, i + 1):
        for wj in range(j + 1 - window_size, j + 1):
...
hsv = e2e_hsv_to_rgb(img)
v_arr = hsv[:, :, 2]
v_arr = np.ascontiguousarray(v_arr)

window_size = 4 # including the target pixel (3 + 1)
