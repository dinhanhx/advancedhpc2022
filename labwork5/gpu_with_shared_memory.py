...
def conv2d(src, dst, k):
    ...
    k_h, k_w = 7, 7
    s_k = cuda.shared.array(shape=(k_h, k_w), dtype=types.float32)
    if cuda.threadIdx.x < k_h and cuda.threadIdx.y < k_w:
        s_k[cuda.threadIdx.x, cuda.threadIdx.y] = k[cuda.threadIdx.x, cuda.threadIdx.y]
    cuda.syncthreads()
    for c in range(3):
        temp = 0
        for m in range(-(k_h//2), k_h//2 + 1):
            for n in range(-(k_w//2), k_w//2 + 1):
                if i + m >= 0 and j + n >= 0 and i + m < src_h and j + n < src_w:
                    temp += np.float32(src[i+m, j+n, c]) * s_k[m + k_h // 2, n + k_w // 2]
        dst[i, j, c] = temp
